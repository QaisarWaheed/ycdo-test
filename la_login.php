...
if ($role_id == 18 || $role_id == 0) 
        {      
            ...
            mysqli_query($con, "INSERT INTO `logins_detail_fr_mm_sm`
              ...
              $_SESSION['lab_admin_user_id'] = $user_id;
              $_SESSION['role_id'] = $role_id;
              $_SESSION['lab_admin_login_branch_id'] = $branch_id;
              $_SESSION['lab_admin_login_is_admin'] = $is_admin;
              $_SESSION['lab_admin_login_is_incharge'] = $is_incharge;
              $_SESSION['lab_admin_login_branch_name'] = $branch_name;
              $_SESSION['lab_admin_login_branch_address'] = $branch_address;
              $_SESSION['lab_admin_login_branch_phone'] = $branch_phone;
              $_SESSION['lab_admin_user_name'] = $user_name;
             header('Location: la/dashboard.php');
              mysqli_close($con);
              exit(0);               
        }          
...
$user = "SELECT * FROM users WHERE (role_id = 0 AND branch_id = 0 AND status = '1') OR (role_id = 18 AND status = '1' AND branch_id = '$branch_id') ORDER BY `u_name` ASC ";
$run_user = mysqli_query($con, $user);
if (mysqli_num_rows($run_user) > 0) 
{
    while ($row_user = mysqli_fetch_array($run_user)) {
...
{
    echo '<option value="">Add Medicine Manager Data</option>';
}
?>
                </select>
