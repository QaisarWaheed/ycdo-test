...
if (isset($_GET['del_medicine']) && $_GET['del_medicine'] !== '') {
    ...
    if (mysqli_query($con, $update)) {
        header('Location: usg.php?token_id=' . urlencode($token_id));
        exit;
    }
}
...
if (isset($_GET['save_test'])) {
    ...
    if ($reg_item_id < 1) {
        header('Location: usg.php?token_id=' . urlencode($token_id) . '&msg=ERROR-INVALID-ITEM');
        exit;
    }
    ...
    if (mysqli_query($con, $insert)) {
        ...
        header('Location: usg.php?token_id=' . urlencode($token_id) . '&days=' . urlencode((string) $days));
        exit;
    }
}
...
<div class="row" style="margin: 0px;">
    ...
    <div class="col-md-12">
        <form autocomplete="off">
            <div class="row">
                <div class="col-md-9">
                    <label><h3>ENTER TOKEN # FOR USG</h3></label>
