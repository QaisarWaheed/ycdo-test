...
$output .=  '<h1 style = "font-size: 25px;" align="center"> '.$id.' / '.date('y').'</h1>
</strong></caption>
    ...
    <tr>
        ...
        <td class="description">'.$_SESSION['fr_name'].'</td>
    </tr>
    <tr>
...
$output .=  '<table>
    ...
    <tr>
        <td class="quantity">Duplicate By:</td>
        <td class="description">'.$GLOBALS['fr_name'].'</td>
    </tr>
    <tr>
        <td>Print Time</td>
...
function next_collection_id($branch_id)
{
    $output = '';
    $run = mysqli_query($GLOBALS['con'], "SELECT fr_collection_id, book_id, slip_no FROM fr_collection WHERE fr_collection_status = '1' AND book_id IN (SELECT `book_no` FROM `receipt_books` WHERE `branch_id` = '$branch_id') ORDER BY fr_collection_id Asc");
    if (mysqli_num_rows($run) > 0) {
        while ($row = mysqli_fetch_array($run)) 
        {
            $output .= '<option value = "'.$row['0'].'">'.$row['2'].' B->'.$row['1'].' </option>';
        }
    }
    else{
        return 1;
