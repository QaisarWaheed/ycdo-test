...
$summary_br_selected = (int) ($_GET['br_id'] ?? $branch_id);
echo fr_branch_select_options($con, (int) $branch_id, (int) $is_admin, (int) $is_incharge, $summary_br_selected, 'br_id');
?>
					</select>
				</div>
				<div class="col-md-6 col-sm-6 col-xs-12">
...
if ($run_user && mysqli_num_rows($run_user) > 0) {
	while ($row_user = mysqli_fetch_array($run_user)) {
		echo '<option value="' . (int) $row_user['id'] . '">' . htmlspecialchars($row_user['u_name'], ENT_QUOTES, 'UTF-8') . '</option>';
	}
} else {
