...
if ($role_id == 11 ||$role_id == 0) 
        {   
            ...
            mysqli_query($con, "INSERT INTO `logins_detail_fr_mm_sm`
                (`user_id`, `created`, `device_ip_address`) VALUES 
                ('$user_id', '$currentdate', '$ip_address')");
              $_SESSION['bk_id'] = $user_id;
...
$user = "SELECT * FROM users WHERE role_id IN (0,11) AND status = '1' AND branch_id = '0' ORDER BY `u_name` ASC ";
$run_user = mysqli_query($con, $user);
if (mysqli_num_rows($run_user) > 0) 
{
    while ($row_user = mysqli_fetch_array($run_user)) {
