...
$users = "SELECT * FROM `summary_details` WHERE login_id IN (SELECT id FROM logins_detail WHERE login_at <= '$last_date' AND `login_at` >= '$from_date' AND `status` = '2') ORDER BY `created` ";
...
$select = "SELECT distinct tokan_type_id ,cash_received FROM tokans WHERE 
	(`created` like '$to_date%' AND tokan_type_id < 100 ) OR 
	(`created` <= '$to_date' AND `created` >= '$from_date' AND tokan_type_id < 100 )
	...
	while ($row = mysqli_fetch_array($run)) 
	{
		$tokan_type_id = $row['tokan_type_id'];
			$select_count = "SELECT * FROM tokans WHERE 
            	(`created` like '$to_date%' AND tokan_type_id = '$tokan_type_id' AND `status` = '1') OR 
	(`created` <= '$to_date' AND `created` >= '$from_date' AND tokan_type_id = '$tokan_type_id' AND `status` = '1' ) ";
