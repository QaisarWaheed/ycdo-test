...
echo '<html><head><meta charset="utf-8"><title>Token Summary</title></head><body><p>Loading token summary…</p>';
...
$detailRun = fr_summary_tokans_detail_result($con, $from_date, $to_date, $u_id, $br_id, false);
?>
<!DOCTYPE html>
<html>
<head>
...
<style>* { font-size: 16px; text-transform: uppercase; }</style>
</head>
...
if ($detailRun) {
    while ($row = mysqli_fetch_assoc($detailRun)) {
        ...
        echo '<tr>
            ...
            <td>' . htmlspecialchars(ycdo_safe_date_format($token_date, 'd M', '')) . '</td>
...
<?php
fr_render_summary_type_breakdown_rows($con, $from_date, $to_date, $u_id, $br_id, false);
if ($u_id === 0) {
    fr_render_summary_branch_extras($con, $from_date, $to_date, $br_id, false);
}
?>
	</tbody>
</table>
