...
if ($role_id == 19 || $role_id == 0) 
        {      
            ...
            mysqli_query($con, "INSERT INTO `logins_detail_fr_mm_sm`
              ...
              $_SESSION['lab_manager_user_id'] = $user_id;
              $_SESSION['role_id'] = $role_id;
              $_SESSION['lab_manager_login_branch_id'] = $branch_id;
              $_SESSION['lab_manager_login_is_admin'] = $is_admin;
              $_SESSION['lab_manager_login_is_incharge'] = $is_incharge;
              $_SESSION['lab_manager_login_branch_name'] = $branch_name;
...
$user = "SELECT * FROM users WHERE (role_id IN (0, 19) AND branch_id = 0 AND status = 1) OR (role_id = 19 AND status = 1 AND branch_id = '$branch_id') ORDER BY `u_name` ASC ";
$run_user = mysqli_query($con, $user);
if (mysqli_num_rows($run_user) > 0) 
{
    while ($row_user = mysqli_fetch_array($run_user)) {
