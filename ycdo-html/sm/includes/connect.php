...
{
    $quanity = 0;
    $run = mysqli_query($GLOBALS['con'], "SELECT dose, feed, days, fix_dose FROM `item_by_doctor` WHERE `item_id` IN (SELECT id FROM item_register_to_branches WHERE item_id = '$item_id') AND `created` LIKE '2024-01%' ");
    if (mysqli_num_rows($run) == 1) 
    {
        while ($row = mysqli_fetch_array($run)) 
        {
            if($row['fix_dose'] == '0')
            {
                $quanity = $quanity + ($row['dose']*$row['feed']*$row['days']);
            }
            else
            {
                $quanity = $quanity+$row['fix_dose'];
            }
...
function get_all_branchs_stock($item_id)
{
    $quanity = 0;
    $run = mysqli_query($GLOBALS['con'], "SELECT SUM(`quantity`) FROM `item_register_to_branches` WHERE `item_id` = '$item_id' ");
    if (mysqli_num_rows($run) == 1) 
    {
        while ($row = mysqli_fetch_array($run)) 
        {
...
function get_branch_item_id_by_item($item_id, $branch_id)
{
    $output = 0;
    $select = "SELECT id FROM `item_register_to_branches` WHERE `branch_id` = '$branch_id' AND `item_id` = '$item_id' ";
    $run = mysqli_query($GLOBALS['con'], $select);
    if (mysqli_num_rows($run) == 1) 
    {
        while ($row = mysqli_fetch_array($run)) 
        {
...
function get_branch_id_by_register_item_id($id)
{
    $output = '';
    $run = mysqli_query($GLOBALS['con'], "SELECT branch_id FROM `item_register_to_branches` WHERE `id` = '$id' ");
    if (mysqli_num_rows($run) == 1) 
    {
        while ($row = mysqli_fetch_array($run)) 
        {
...
{
    $output = '';
    $select = "SELECT tag_name FROM `branchs` WHERE `id` = '$id' ";
    $run = mysqli_query($GLOBALS['con'], $select);
    if (mysqli_num_rows($run) == 1) 
    {
        while ($row = mysqli_fetch_array($run)) 
...
function get_receive_quantity_from_branch($id)
{
    $quanity = 0;
    $run = mysqli_query($GLOBALS['con'], "SELECT `quantity` FROM `return_item_by_branch` WHERE `id` = '$id' ");
    if (mysqli_num_rows($run) == 1) 
    {
        while ($row = mysqli_fetch_array($run)) 
        {
...
function get_item_quantity_by_item_id($id)
{
    $quanity = 0;
    $run = mysqli_query($GLOBALS['con'], "SELECT quantity FROM items WHERE id = '$id' ");
    if (mysqli_num_rows($run) == 1) 
    {
        while ($row = mysqli_fetch_array($run)) 
        {
...
function show_category_name_by_item_id($item_id)
{
    $output = '';
    $run = mysqli_query($GLOBALS['con'], "SELECT * FROM `categories` WHERE id IN (SELECT category_id FROM items WHERE id = '$item_id') ");
    if (mysqli_num_rows($run) > 0) {
        while ($row = mysqli_fetch_array($run)) 
        {
            $name = $row['name'];
...
{
    $output = '';
    $get_patient = mysqli_query($GLOBALS['con'], "SELECT * FROM items WHERE id = '$item_id' ");
    if (mysqli_num_rows($get_patient) == 1) 
    {
        while ($row_patient = mysqli_fetch_array($get_patient)) 
        {
