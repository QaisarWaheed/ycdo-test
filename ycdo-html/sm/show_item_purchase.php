...
if (mysqli_num_rows($select_purchase) > 0) {
	while ($row_purchase = mysqli_fetch_array($select_purchase)) {
		...
		if($row_purchase['mfg_date'] == '0000-00-00'){$mfg_date_display = "";}else{$mfg_date_display = date_format(date_create($row_purchase['mfg_date']), "d-m-y");}
		if($row_purchase['expiry_date'] == '0000-00-00'){$expiry_date_display = "";}else{$expiry_date_display = date_format(date_create($row_purchase['expiry_date']), "d-m-y");}
...
<form method="POST">
	...
	<input type="number" name="receive_quantity" style="max-width: 100px;">
	<input type = "submit" value = "Receive" name="save_receive_quantity" style="btn btn-info">
	</td>
	<td>'.$status_msg.'</td>
