...
if(isset($_GET['audit_store_form_id']) && $_GET['audit_store_form_id'] > '4')
{
...
if(isset($_GET['clear_id']))
{
    ...
    $update = "UPDATE `audit_store_detail` SET `audit_store_detail_updated_by` = '$user_id', `audit_store_detail_updated_at` = '$current_date' WHERE `audit_store_detail_id` = '$clear_id' AND `audit_store_detail_updated_by` = '0' ";
    ...
    header('Location: audit_store_clear.php?audit_store_form_id='.$audit_store_form_id);
    exit;
}
            ...
            <tbody>
                ...
                if(mysqli_num_rows($run) > 0)
                {
                    while($row = mysqli_fetch_array($run))
                    {
                        ...
                        $audit_store_detail_id = $row['audit_store_detail_id'];
                        $item_id = $row['item_id'];
                        $name = $row['name'];
                        ...
                        <tr>
                            ...
                            <td>'.$difference.'</td>
                            <td>
                                <a href = "audit_store_clear.php?clear_id='.$audit_store_detail_id.'&quantity='.$difference.'&item_id='.$item_id.'&audit_store_form_id='.$audit_store_form_id.'">CLEAR</a>
                            </td>
                        </tr>
                        ';    
                    }
