...
.noprint
{
	...
	<title>AUDIT LAB DIFERENCE REPORT - <?php echo $company_trademark; ?></title>
</head>
    ...
    <div class = "col-md-12">
        <table  class = "table table-bordered table-hover">
            ...
            <tbody>
                ...
                $total_extra = 0;
                $total_short = 0;
                $select = "SELECT categories.name AS cat_name, items.name AS name, audit_lab_store_detail.audit_lab_store_detail_created_at AS created, audit_lab_store_detail.poor AS poor, audit_lab_store_detail.computer_stock AS computer_stock, audit_lab_store_detail.manual_stock AS manual_stock, audit_lab_store_detail.manual_tries AS manual_tries FROM `audit_lab_store_detail` INNER JOIN items ON audit_lab_store_detail.item_id = items.id INNER JOIN categories ON items.category_id = categories.id WHERE audit_lab_store_form_id = '$audit_lab_store_form_id' AND computer_stock != manual_stock;  ";
                ...
                if(mysqli_num_rows($run) > 0)
                {
                    while($row = mysqli_fetch_array($run))
                    {
                        ...
                        {
                            $s = $s + 1;
                            $total_short = $total_short + abs($poor*$difference);
                        echo '
                        <tr>
                            <td>'.$s.'</td>
                            <td>'.$name.' - '.$cat_name.'</td>
                            <td>'.$poor.'</td>
                            <td>'.$difference.'</td>
                            <td>'.abs($poor*$difference).'</td>
                            <td></td>
                            <td></td>
                        </tr>
                        ';    
                        }
                        elseif($computer_stock < $manual_stock)
                        {
                            $s = $s + 1;
        ...
        <div style = "font-size: 24px;text-align: center;color: black;">
            <p>EXTRA ITEMS AMOUNT: <span> <?php echo $total_extra; ?></span></p>
            <p>SHORT ITEMS AMOUNT: <span> <?php echo $total_short; ?></span></p>
