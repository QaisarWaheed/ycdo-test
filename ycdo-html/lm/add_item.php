...
if (isset($_POST['save'])) {
	...
	$run2 = "INSERT INTO `items`
	(`category_id`, `name`, `barcode`, `retail`, `deserving`, `poor`, `member`, `general`, `min_limit`, `max_limit`, `created`, `user_id`) 
	VALUES 
	('$category_id', '$name', '$barcode', '$retail', '$deserving', '$poor', '$member', '$general', '$min_limit', '$max_limit', '$current_date', '$lab_manager_user_id')";
	if(mysqli_query($con, $run2))
	{
					...
					<div class="row">
						...
						<div class="col-md-12">
							...
							<select name="category_id" class="form-control" required>
								...
								<?php
								$categories = mysqli_query($con, "SELECT * FROM `categories` WHERE id IN (2, 7 ,28 ,43) AND status = '1' ORDER BY name");
						...
						<div class="col-md-12" style="margin: 20px 0px;">
							...
							<a href="show_items.php" class="btn btn-info">SHOW ITEMS</a>
                        <?php if($is_admin == 2){ ?>
                            <a href="show_items_with_rate.php" class="btn btn-danger">SHOW ITEMS WITH RATE</a>
							<a href="show_item_quantity.php" class="btn btn-primary">SHOW ITEMS(QUANTITY)</a>
							<a href="update_item_store_limits.php" class="btn btn-primary">UPDATE ITEMS(STORE LIMIT)</a>
                        <?php } ?>
						</div>
					</div>
