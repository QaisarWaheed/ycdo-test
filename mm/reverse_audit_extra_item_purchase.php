...
if (isset($_POST['save'])) 
{
    ...
    if(mysqli_query($con, $update))
    {
        ...
        echo "<br>";
        echo '<br><a href = "reverse_audit_extra_item_purchase.php?audit_no='.$audit_id.'">BACK</a>';
        exit(0);
    }
    header('location: reverse_audit_extra_item_purchase.php?audit_no='.$audit_id);
}
	...
	<div class="" style="margin: 10px 15px;">
		<div class="row">
			...
			<div class="col-md-12" style="text-align: center;">
				<label><h1>Audit extra Item List</h1></label>
				<div class = "table">
...
if(mysqli_num_rows($run) > 0)
{
    while($row = mysqli_fetch_array($run))
    {
        ...
        if($clear_status == 2)
        {
            echo '<td class="noprint"><a href = "reverse_audit_extra_item_purchase.php?save='.$id.'&audit_id='.$audit_id.'" class = "btn btn-sm btn-warning">
                <form method = "POST">
                    <input type = "hidden" name = "audit_id" value = "'.$audit_id.'" />
                    <input type = "hidden" name = "record_id" value = "'.$id.'" />
                    <input type = "hidden" name = "branch_item_id" value = "'.$branch_item_id.'" />
                    <input type = "hidden" name = "quantity" value = "'.$quantity.'" />
                    <input type = "submit" class = "btn btn-info btn-sm" name = "save" value = "REVERSE" />
                </form>
            </td>';
        }
        else
