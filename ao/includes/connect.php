...
if (empty($_SESSION['ao_id'])) {
    header('Location: logout.php');
    exit;
}

$user_id = (int) $_SESSION['ao_id'];
$user_name = $_SESSION['ao_name'] ?? '';
...
function get_poor_price($item_id)
{
    $output = '';
    $run = mysqli_query($GLOBALS['con'], "SELECT poor FROM `items` WHERE `id` = '$item_id' ");
    if (mysqli_num_rows($run) == 1) 
    {
        while ($row = mysqli_fetch_array($run)) 
        {
...
{
    $output = '';
    $run = mysqli_query($GLOBALS['con'], "SELECT * FROM `categories` WHERE status = '1' ORDER BY `categories`.`name` ASC ");
    if (mysqli_num_rows($run) > 0) {
        while ($row = mysqli_fetch_array($run)) 
        {
            $id = $row['id'];
...
function get_audit_id_from_branch_item_id($branch_item_id, $audit_id)
{
    $output = 0;
    $run = mysqli_query($GLOBALS['con'], "SELECT id FROM `audit_branch_detail` WHERE `branch_item_id` = '$branch_item_id' AND audit_id = '$audit_id' ");
    if (mysqli_num_rows($run) == 1) 
    {
        while ($row = mysqli_fetch_array($run)) 
        {
...
function get_audit_price_from_branch_item_id($branch_item_id, $audit_id)
{
    $output = 0;
    $run = mysqli_query($GLOBALS['con'], "SELECT item_poor_price FROM `audit_branch_detail` WHERE `branch_item_id` = '$branch_item_id' AND audit_id = '$audit_id' ");
    if (mysqli_num_rows($run) == 1) 
    {
        while ($row = mysqli_fetch_array($run)) 
        {
            $output = $row['item_poor_price'];
        }    
    }    
    return $output;
}
...
function get_audit_tries_from_branch_item_id($branch_item_id, $audit_id)
{
    $output = 0;
    $run = mysqli_query($GLOBALS['con'], "SELECT tries FROM `audit_branch_detail` WHERE `branch_item_id` = '$branch_item_id' AND audit_id = '$audit_id' ");
    if (mysqli_num_rows($run) == 1) 
    {
        while ($row = mysqli_fetch_array($run)) 
        {
...
function get_audit_manual_from_branch_item_id($branch_item_id, $audit_id)
{
    $output = 0;
    $run = mysqli_query($GLOBALS['con'], "SELECT manual_quantity FROM `audit_branch_detail` WHERE `branch_item_id` = '$branch_item_id' AND audit_id = '$audit_id' ");
    if (mysqli_num_rows($run) == 1) 
    {
        while ($row = mysqli_fetch_array($run)) 
        {
            $output = $row['manual_quantity'];
        }    
    }    
    return $output;
}
