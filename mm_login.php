...
if ($role_id == 6 || $role_id == 0) 
        {      
            ...
            mysqli_query($con, "INSERT INTO `logins_detail_fr_mm_sm`
                ...
                ('$user_id', '$currentdate', '$ip_address', '6', '$branch_id')");
...
$user = "SELECT * FROM users WHERE (role_id = 0 AND branch_id = 0 AND status = '1') OR (role_id = 6 AND status = '1') ORDER BY `u_name` ASC ";
$run_user = mysqli_query($con, $user);
if (mysqli_num_rows($run_user) > 0) 
{
    while ($row_user = mysqli_fetch_array($run_user)) {
