...
// Defaulting to current month based on system time
...
$sql = "SELECT 
            ...
            i_stats.ecgs,
            -- Referral Metrics
            IFNULL(r_stats.referral_from, 0) AS referral_from,
            IFNULL(r_stats.referral_to, 0) AS referral_to,
        ...
        FROM (
            /* Aggregate Financials and OPD from tokans */
            SELECT doctor_id, 
        ...
        LEFT JOIN (
            /* Aggregate all 15 categories from item_by_doctor */
            SELECT doctor_id,
        ...
        LEFT JOIN (
            ...
            SELECT 
                d.doctor_id,
                SUM(d.is_from) AS referral_from,
                SUM(d.is_to) AS referral_to
            FROM (
                SELECT from_user_id AS doctor_id, 1 AS is_from, 0 AS is_to FROM referral_patients 
                WHERE (referral_patient_created >= '$first_day 00:00:00' AND referral_patient_created <= '$last_day 23:59:59') AND branch_id = '$branch_id'
                UNION ALL
                SELECT to_user_id AS doctor_id, 0 AS is_from, 1 AS is_to FROM referral_patients
                WHERE (referral_patient_created >= '$first_day 00:00:00' AND referral_patient_created <= '$last_day 23:59:59') AND branch_id = '$branch_id'
            ) d
            GROUP BY d.doctor_id
        ) AS r_stats ON t_stats.doctor_id = r_stats.doctor_id
        LEFT JOIN (
            SELECT doctor_id, COUNT(id) AS gynae_registrations
            FROM gynae_register
            WHERE (created >= '$first_day 00:00:00' AND created <= '$last_day 23:59:59') AND branch_id = '$branch_id'
            GROUP BY doctor_id
        ) AS g_stats ON t_stats.doctor_id = g_stats.doctor_id
