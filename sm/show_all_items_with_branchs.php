...
<?php
if(isset($_POST['update_locked_status']))
{
    $branch_register_id = $_POST['branch_register_id'];
    $is_locked = $_POST['is_locked'];
    $query = "UPDATE `item_register_to_branches` SET `is_locked` = $is_locked WHERE `id` = $branch_register_id ";
...
?>
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            Swal.fire({
                title: 'Success!',
                text: 'Your value has been updated.',
                icon: 'success',
    ...
    while($row_br = mysqli_fetch_array($run_br))
    {
		...
		echo '
				<tr>
					...
					<td>'; ?>
					<form method = "POST">
					    <input type = "hidden" value = "<?php echo $branch_register_id; ?>" name = "branch_register_id" />
					    <select class = "form-control" name =  "is_locked" required>
					        <option <?php if($br_is_locked == 1){echo ' SELECTED';} ?> value = "1">LOCKED</option>
					        <option <?php if($br_is_locked == 0){echo ' SELECTED';} ?>  value = "0">OPEN</option>
					    </select>
					    <input type = "submit" value = "UPDATE" name = "update_locked_status" />
					</form>
					<?php echo '</td>
					<td>'.$br_quantity.'</td>
					<td>'.$br_min_limit.'</td>
